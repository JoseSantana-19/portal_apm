...
<div class="card">
    <div class="table-responsive-wrapper">
        <table id="usr-table" data-dt data-dt-page-length="25">
            <thead>
                <tr>
                    <th>Cédula</th>
            ...
            <tr>
                <td>
                    <div style="display:flex;align-items:center;gap:var(--sp-2);">
                        ...
                        <?php if ($isSelf): ?><span class="badge badge-info" style="font-size:0.65rem;">Tú</span><?php endif; ?>
                    </div>
                </td>
                <td style="font-weight:var(--font-weight-medium);"><?= htmlspecialchars($u['nombre_completo'], ENT_QUOTES, 'UTF-8') ?></td>
                <td style="color:var(--color-text-muted);"><span class="dt-truncate dt-truncate-sm" title="<?= htmlspecialchars($u['correo'] ?? '', ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($u['correo'] ?? '—', ENT_QUOTES, 'UTF-8') ?></span></td>
                <td><span class="badge <?= $nivelColor ?>"><?= $nivelLbl ?></span></td>
                <td><span class="dt-truncate dt-truncate-sm" title="<?= htmlspecialchars($u['departamento'] ?? '', ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($u['departamento'] ?? '—', ENT_QUOTES, 'UTF-8') ?></span></td>
                <td>
                    ...
                    <div class="dt-actions">
                        ...
                        <a href="<?= APP_URL ?>/admin/usuarios/<?= $u['id_usuario'] ?>/export/excel"
                           class="btn btn-ghost btn-sm" title="Exportar usuario a Excel">
                            <i class="fa-solid fa-file-excel" style="color:#1D6F42;"></i>
                        </a>
                        <?php if (!$isSelf && $u['estado']): ?>
                        <form method="POST" action="<?= APP_URL ?>/admin/usuarios/<?= $u['id_usuario'] ?>/eliminar">
                            <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrf, ENT_QUOTES, 'UTF-8') ?>">
                            <button type="button" class="btn btn-ghost btn-sm" style="color:var(--color-danger);" title="Desactivar usuario"
                        ...
                        <?php elseif (!$u['estado']): ?>
                        <form method="POST" action="<?= APP_URL ?>/admin/usuarios/<?= $u['id_usuario'] ?>/activar">
                            <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrf, ENT_QUOTES, 'UTF-8') ?>">
                            <button type="button" class="btn btn-ghost btn-sm" style="color:var(--color-success);" title="Activar usuario"
