...
<title>Confirmar identidad — Portal APM</title>
...
:root {
    ...
    --bg-page: #F8FAFC; --bg-card: rgba(255, 255, 255, 0.94); --bg-input: #F1F5F9;
    --border-color: #E2E8F0; --text-main: #0F172A; --text-muted: #64748B; --text-sub: #334155;
    --shadow-card: 0 24px 48px -12px rgba(15, 23, 42, 0.12); --r: 14px;
...
.orb-1 { width: 380px; height: 380px; background: #10B981; top: -100px; left: -100px; }
...
.card-top-accent { position: absolute; top: -2px; left: 20px; right: 20px; height: 4px; background: linear-gradient(90deg, #10B981, #0284c7, #8B5CF6); border-radius: 99px; }

.mfa-icon { width: 64px; height: 64px; border-radius: 18px; background: linear-gradient(135deg, #10B981, #0284c7); display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; box-shadow: 0 12px 28px rgba(16,185,129,.3); }
.mfa-icon i { font-size: 26px; color: #fff; }

.heading { text-align: center; font-size: 22px; font-weight: 800; color: var(--text-main); letter-spacing: -.02em; margin-bottom: 6px; }
...
.code-boxes input:focus { border-color: #10B981; background: var(--bg-card); box-shadow: 0 0 0 4px rgba(16,185,129,.15); }
...
.btn-verify {
    ...
    background: linear-gradient(135deg, #10B981 0%, #0284c7 100%); box-shadow: 0 8px 24px rgba(16,185,129,.3);
    display: flex; align-items: center; justify-content: center; gap: 10px; transition: var(--ease);
}
.btn-verify:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 12px 32px rgba(2,132,199,.4); }
...
<div class="scene">
    <div class="card">
        ...
        <div class="mfa-icon"><i class="fa-solid fa-fingerprint"></i></div>
        <div class="heading">Confirma tu identidad</div>
        <div class="sub">Por seguridad, este módulo pide reconfirmar tu código de verificación antes de entrar.</div>
        ...
        <form method="POST" action="<?= APP_URL ?>/ir/verificar" id="mfa-form" autocomplete="off">
        ...
        <a href="<?= APP_URL ?>/dashboard" class="back-link"><i class="fa-solid fa-arrow-left-long"></i> Cancelar y volver al Dashboard</a>
    </div>
</div>
