...
public function GetStringWidth($s)
{
	...
	$l = strlen($s);
	for($i=0;$i<$l;$i++)
		$w += $cw[$s[$i]] ?? 500; // 500 = default width for unmapped chars (ISO-8859-1 extended)
...
protected function _loadjsonfont($path)
{
	...
	// Normalise character widths: JSON keys are numeric strings, FPDF expects chr() keys
	$cw = array();
	foreach($info['cw'] as $c=>$w)
		$cw[chr((int)$c)] = $w;
	// Fill missing entries for ISO-8859-1 extended characters (127-255) with default width
	// This prevents "Undefined array key" warnings when rendering accented characters
	for($i=0; $i<=255; $i++)
	{
		$ch = chr($i);
		if(!isset($cw[$ch]))
			$cw[$ch] = 500; // reasonable default width for unmapped glyphs
	}
	$info['cw'] = $cw;
	if(isset($info['enc']))
	...
	// Normalise type to match expected values: 'Core', 'Type1', 'TrueType'
	if(isset($info['type']))
		$info['type'] = ucfirst(strtolower($info['type']));
	return $info;
}
