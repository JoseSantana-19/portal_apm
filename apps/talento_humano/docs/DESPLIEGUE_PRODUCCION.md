...
Asistencia, Vacaciones, Desempeño y Capacitación permanecen como prototipos por decisión funcional. La página 4 socioeconómica permanece reservada hasta recibir el formato autorizado. Ninguno de estos componentes debe considerarse fuente oficial.
...
3. Aplicar exactamente las migraciones vigentes y en el orden documentado en `database/README.md`. La última debe ser `database/migracion_cierre_produccion_20260806.sql`.
...
- Los PDF oficiales conservan dos páginas para Acción de Personal y cuatro para socioeconómico, con la cuarta marcada como pendiente.
...
La activación efectiva del cifrado SQL requiere el certificado del servidor y normalmente reiniciar el servicio SQL Server. Debe coordinarse con TI antes de cambiar `PORTAL_DB_ENCRYPT` para no interrumpir el portal.
