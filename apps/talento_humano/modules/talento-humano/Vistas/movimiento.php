...
<div class="app">
    ...
    <section class="content">
        <?php $topbarTitle='Movimiento interno';$topbarSubtitle='Sin Acción de Personal';$topbarShowSearch=true;$topbarBackUrl=BASE_URL.'/talento-humano/directorio?modo=movimiento';$topbarBackLabel='Volver';require ROOT.'/shared/topbar.php'; ?>
        <main class="main"><div class="content-shell">
            <section class="card">
                ...
                <form class="movement-grid" method="post" action="<?= BASE_URL ?>/talento-humano/empleado/mover" data-draft-context="movimiento:<?= (int)$empleado['empleado_id'] ?>">
                    ...
                    <div class="movement-field"><label for="unidad_destino_id">Área / Dirección de destino</label><select id="unidad_destino_id" name="unidad_destino_id" required data-searchable-select data-search-placeholder="Buscar área de destino…"><option value="">Seleccione...</option><?php foreach ($areas as $area): ?><option value="<?= (int)$area['unidad_id'] ?>"><?= htmlspecialchars(($area['unidad_padre_id'] ? '↳ ' : '') . $area['nombre_unidad']) ?></option><?php endforeach; ?></select></div>
                    <div class="movement-current"><i class="bi bi-lock-fill"></i> El cargo <strong><?= htmlspecialchars($empleado['cargo'] ?? 'Sin asignar') ?></strong> se conservará en el área de destino.</div>
