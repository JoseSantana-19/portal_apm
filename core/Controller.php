...
abstract class Controller {
    ...
    protected function requireAuth(?string $module = null): void {
        ...
        // usuario > módulo > global, configurable en /admin/inactividad).
    ...
    /**
     * [segundos, aviso_segundos] efectivos para el usuario actual en este
     * módulo ('CENTRAL'). Cacheado en sesión 5 minutos para no consultar la
     * BD en cada request — un cambio del admin tarda hasta ese margen en
     * reflejarse en una sesión ya abierta.
     */
    protected function resolveInactividad(): array {
        ...
        try {
            ...
            $stmt = sqlsrv_query($db->getConn(),
                ...
                [
                    [$idUsuario, SQLSRV_PARAM_IN], ['CENTRAL', SQLSRV_PARAM_IN],
                    [$idUsuario, SQLSRV_PARAM_IN], ['CENTRAL', SQLSRV_PARAM_IN],
                ]
            );
            if ($stmt !== false) {
